...
def print_state(active_cubes):
    ...
    for z in range(min(z_s), max(z_s)+1):
        ...
        for y in range(min(y_s), max(y_s)+1):
            print("".join(["#" if (x,y,z) in active_cubes else "." for x in range(min(x_s), max(x_s)+1)]))
        print()


def active_neighbours(coord, state):
    (x, y, z) = coord
    combinations = list(itertools.product(list(range(-1, 2)), list(range(-1, 2)), list(range(-1, 2))))
    combinations.remove((0, 0, 0))
    targets = list(map(lambda t: (x + t[0], y + t[1], z + t[2]), combinations))
